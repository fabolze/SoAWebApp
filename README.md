...
- `NARRATIVE_CREATION_FLOW_PLAN.md`: canonical product contract, implementation record, delivery state, and external handoff plan for capture-first mixed-content sequences and the contextual “Then…” composer.
- `NARRATIVE_CREATION_FLOW_WORKFLOWS.md`: canonical original-author workflow corpus, derived requirements, W1/W2/W3 acceptance scenarios, and repository evidence.
...
- `/playtest` provides **Shadows of Shanoir**, a self-contained browser vertical slice based on the current story documents: Raznah and Roznoh's separation, the player's childhood portal encounter, a missing villager, and the first active Shadow incident. It includes a prologue, branching dialogue, quest progression, inventory and equipment, a shop, gated world travel, real-time top-down party combat, talents, lore discovery, procedural music, local save/continue, and an original painted Altrail environment plate. Combat uses WASD/arrow movement, dual ally/enemy targeting, party frames, boss cast bars, avoidable line/circle/cleave telegraphs, raid-wide damage, `1`–`4` damage and support abilities, and `Space` to dodge. Names and identities still marked open in the source material remain explicit placeholders in the playtest. Progress remains browser-local and never mutates authoring records.
...
- Narrative Creation Flow capture from Dialogue, World Builder, Encounter, Quest, Event, and POI contexts plus the standalone Creation Flow workspace. Mixed-content **Then…** sequences and **Expand this place** constellations support insertion, duplication, undo/redo, placeholders, prose-linked idea cards, branch-aware rehearsal, snapshots, and JSON recovery. Unfinished drafts remain browser-local; supported steps resolve canonical targets, preview rollback-only, commit atomically, and persist a source-recoverable authoring manifest excluded from UE/DataTable exports.
