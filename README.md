...
- Narrative Creation Flow capture from dialogue choices/endings and selected World Builder locations, including mixed-content **Then…** sequences, **Expand this place** constellations, placeholders, prose-linked idea cards, snapshots, and JSON recovery. Unfinished drafts remain browser-local; the bounded Phase 2 subset can resolve canonical targets, run rollback-only preview/rehearsal, commit atomically, and persist a source-recoverable authoring manifest excluded from UE/DataTable exports.
