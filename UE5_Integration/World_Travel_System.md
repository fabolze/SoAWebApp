...
| `TravelPlan` | Full planner output (runtime-only, segments + totals + diagnostics). | Planner returns structured failures (missing requirements, unreachable, unaffordable). |
| `TravelSessionState` | Persistable runtime state for pause/resume (runtime-only). | Needed if encounters/events load other maps or the player saves mid-travel. |
...
- Output: `TravelPlan` containing segments, totals, and structured diagnostics (why a plan failed).
...
- Executes `TravelPlan` step by step:
  ...
  3. Trigger forced events (`RouteEventBinding`) via `BP_EventSequencer`.
  4. Roll random encounters using a deterministic random stream seeded per travel session (formula tuned via `TravelTuningData` + route modifiers + player stats).
...
- Owns `TravelSessionState` so travel can pause/resume across encounters, save/load, or map transitions.
...
- **Safe Zones:** Prefer an explicit route override/tag (e.g. `SafePassage`) or a single rule in `TravelTuningData` (for example "routes connected to safe zones have encounter multiplier = 0"). Avoid duplicating safe-zone state into multiple tables unless you need a deliberate override.
...
- **Automation Tests**
  ...
  - `TravelPlanner_PathConsistency`: fixed data plus seed returns the same `TravelPlan`.
