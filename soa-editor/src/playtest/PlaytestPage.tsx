...
export default function PlaytestPage() {
  ...
  const winEncounter = (remainingHp: number) => {
    ...
    setScreen("game");
    toast(state.location === "ruins" ? "Missing villager found · portal fragment recovered" : "Encounter cleared · the trail continues");
  };
  ...
  const loseEncounter = () => {
    ...
    setScreen("game");
    toast("The search party carried you home · 10 gold lost");
  };
  ...
  const chooseDialogue = (choice: { label: string; next: string | null; action?: string }) => {
    ...
    if (choice.action === "completeQuest") {
      setState((current) => {
        let next = removeItem(current, "portalFragment");
  ...
  return (
    <main className={`pt-game pt-location-${state.location}`}>
      ...
      <aside className="pt-quest-tracker">
        <span className="pt-kicker">Tracked quest</span><h3>{state.questStage === "not-started" ? "Someone Is Missing" : "The Forbidden Path"}</h3><p><i />{objectiveText(state.questStage)}</p><button onClick={() => setPanel("journal")}>Open journal</button>
      </aside>
...
function LocationActions({ state, onDialogue, onShop, onEncounter, onTravel, onLore }: { state: PlayState; onDialogue: (key: DialogueKey) => void; onShop: () => void; onEncounter: () => void; onTravel: () => void; onLore: (id: string) => void }) {
  if (state.location === "village") return <div className="pt-scene-actions">
    <button className="pt-scene-card primary" onClick={() => onDialogue(state.questStage === "return" ? "questReturn" : "questIntro")}><span className="pt-card-icon">V</span><span><small>Village Gathering</small><strong>Village Elder</strong><em>{state.questStage === "return" ? "The rescued villager is safe" : state.questStage === "not-started" ? "A search party has returned empty-handed" : "Name and identity remain open"}</em></span><b>›</b></button>
    <button className="pt-scene-card" onClick={onShop}><span className="pt-card-icon smith">⚒</span><span><small>Village Forge</small><strong>Local Smith</strong><em>Weapons, armor & tonics · Name open</em></span><b>›</b></button>
    <button className="pt-scene-card" onClick={onTravel}><span className="pt-card-icon road">↠</span><span><small>Southern Edge</small><strong>The Forbidden Path</strong><em>{state.questStage === "not-started" ? "Closed until the search begins" : "Open the travel map"}</em></span><b>›</b></button>
  </div>;
  ...
  return <div className="pt-scene-actions wilderness">
    {!cleared ? <button className="pt-scene-card danger primary" onClick={onEncounter}><span className="pt-card-icon">!</span><span><small>{state.location === "ruins" ? "Major encounter" : "Hostile encounter"}</small><strong>{state.location === "forest" ? "Panicked animals block the trail" : state.location === "swamp" ? "Shadow-touched shapes gather" : "A Shadow creature guards the villager"}</strong><em>Enter real-time combat</em></span><b>⚔</b></button> : <button className="pt-scene-card cleared"><span className="pt-card-icon">✓</span><span><small>Area secured</small><strong>{state.location === "ruins" ? "The missing villager is safe" : "The trail is clear"}</strong><em>Threats will not respawn in this playtest</em></span></button>}
...
function Journal({ state }: { state: PlayState }) { return <div className="pt-journal"><div className="pt-chapter-number">I</div><span className="pt-kicker">Main quest · Level 1</span><h3>The Forbidden Path</h3><p>Find the missing villager and discover why the old portal has begun disturbing the land around your home.</p><div className="pt-objective-list">{["Speak with the village elder", "Follow the trail into the forest", "Cross the southern marsh", "Rescue the villager at the portal", "Return to the village"].map((label, index) => { const stageOrder = ["not-started", "reach-forest", "cross-fen", "reach-gate", "return", "complete"]; const current = stageOrder.indexOf(state.questStage); return <div key={label} className={current > index ? "done" : current === index ? "active" : ""}><i>{current > index ? "✓" : index + 1}</i><span>{label}</span></div>; })}</div><footer><span>Rewards</span><b>175 XP</b><b>◆ 75</b><b>Resonant Portal Shard</b></footer></div>; }
