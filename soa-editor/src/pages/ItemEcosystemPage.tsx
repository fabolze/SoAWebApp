...
function ShopSources({ rows: values, shops, itemId, onChange }: { rows: EntryRecord[]; shops: EntryRecord[]; itemId: string; onChange: (rows: EntryRecord[]) => void }) {
  return <section className={panelClass}><PanelTitle title="Shop Inventory" count={values.length} onAdd={() => onChange([...values, { id: generateUlid(), slug: generateSlug(`item-shop-${Date.now()}`), shop_id: shops[0]?.id || "", item_id: itemId, price_modifier: 0, price_multiplier: 1, stock: null, tags: [] }])} />
    <div className="mt-3 space-y-3">{values.map((row, index) => <SourceCard key={text(row.id, String(index))} title={label(shops.find((shop) => shop.id === row.shop_id), text(row.shop_id))} onRemove={() => onChange(values.filter((_, rowIndex) => rowIndex !== index))}>
      <Select label="Shop" value={text(row.shop_id)} options={shops} onChange={(shop_id) => onChange(patchRow(values, index, { shop_id }))} />
...
function NestedSources({ definition, rows: values, catalog, itemId, onChange }: { definition: typeof sourceDefinitions[number]; rows: SourceEntry[]; catalog: EntryRecord[]; itemId: string; onChange: (rows: SourceEntry[]) => void }) {
  return <section className={panelClass}><PanelTitle title={definition.label} count={values.length} onAdd={() => onChange([...values, { owner_id: text(catalog[0]?.id), entry: { item_id: itemId, quantity: 1, ...(definition.chance ? { drop_chance: 100 } : {}) } }])} />
    <div className="mt-3 space-y-3">{values.map((row, index) => <SourceCard key={`${row.owner_id}-${index}`} title={label(catalog.find((owner) => owner.id === row.owner_id), row.owner_id)} onRemove={() => onChange(values.filter((_, rowIndex) => rowIndex !== index))}>
      <Select label="Source" value={row.owner_id} options={catalog} onChange={(owner_id) => onChange(patchSource(values, index, { owner_id }))} />
...
function EconomyPanel({ packet, updateItem, setSources, simulation }: { packet: ItemPacket; updateItem: (patch: EntryRecord) => void; setSources: (key: string, value: unknown) => void; simulation: SimulationResult | null }) {
  ...
  return <div className="grid gap-4 lg:grid-cols-2"><section className={panelClass}><h2 className="font-semibold">Canonical Economy</h2><div className="mt-3 grid gap-3 sm:grid-cols-2"><NumberField label="Base Price" value={packet.item.base_price} onChange={(base_price) => updateItem({ base_price })} /><Select label="Base Currency" value={text(packet.item.base_currency_id)} options={packet.catalogs.currencies || []} allowEmpty onChange={(base_currency_id) => updateItem({ base_currency_id })} /></div><div className="mt-3 grid gap-3 sm:grid-cols-2"><Fact label="Simulation Value" value={simulation?.metrics.value.toFixed(1) || "..."} /><Fact label="Economy Score" value={simulation?.metrics.economy.toFixed(1) || "..."} /></div></section>
...
function Select({ label: fieldLabel, value, options, onChange, allowEmpty = false }: { label: string; value: string; options: EntryRecord[]; onChange: (value: string) => void; allowEmpty?: boolean }) { return <label className="block text-xs font-semibold uppercase text-slate-500">{fieldLabel}<select className={`${inputClass} mt-1 normal-case`} value={value} onChange={(event) => onChange(event.target.value)}>{allowEmpty && <option value="">Unassigned</option>}{options.map((option) => <option key={text(option.id)} value={text(option.id)}>{label(option)}</option>)}</select></label>; }
