...
const panelClass = "rounded-md border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900";
...
export default function ItemEcosystemPage() {
  ...
  return <div className="min-h-full bg-slate-100 p-4 dark:bg-slate-950">
    <div className="mx-auto max-w-7xl space-y-4">
      ...
      <section className={panelClass}><label className="block text-xs font-semibold uppercase text-slate-500">Open Existing Item<select className={`${inputClass} mt-1 normal-case`} value={isNew ? "" : id} onChange={(event) => { if (event.target.value) navigate(`/author/items/${encodeURIComponent(event.target.value)}/ecosystem`); }}><option value="">Select an existing item</option>{(packet.catalogs.items || []).map((item) => <option key={text(item.id)} value={text(item.id)}>{label(item)}</option>)}</select></label></section>
...
function ShopSources({ rows: values, shops, itemId, onChange }: { rows: EntryRecord[]; shops: EntryRecord[]; itemId: string; onChange: (rows: EntryRecord[]) => void }) {
  return <section className={panelClass}><PanelTitle title="Shop Inventory" count={values.length} onAdd={() => onChange([...values, { id: generateUlid(), slug: generateSlug(`item-shop-${Date.now()}`), shop_id: shops[0]?.id || "", item_id: itemId, price_modifier: 0, price_multiplier: 1, stock: null, tags: [] }])} />
    <div className="mt-3 space-y-3">{values.map((row, index) => <SourceCard key={text(row.id, String(index))} title={label(shops.find((shop) => shop.id === row.shop_id), text(row.shop_id))} onRemove={() => onChange(values.filter((_, rowIndex) => rowIndex !== index))}>
      ...
      <div className="grid gap-2 sm:grid-cols-3"><NumberField label="Stock (blank = infinite)" value={row.stock} onChange={(stock) => onChange(patchRow(values, index, { stock }))} /><NumberField label="Multiplier" value={row.price_multiplier} onChange={(price_multiplier) => onChange(patchRow(values, index, { price_multiplier }))} /><NumberField label="Modifier" value={row.price_modifier} onChange={(price_modifier) => onChange(patchRow(values, index, { price_modifier }))} /></div>
      {isRecord(row.pricing) && <div className="text-xs text-slate-500">Current computed buy price: {text(row.pricing.buy_price)}</div>}
    </SourceCard>)}{values.length === 0 && <Empty text="Not sold by any shop." />}</div>
  </section>;
}
...
function NestedSources({ definition, rows: values, catalog, itemId, onChange }: { definition: typeof sourceDefinitions[number]; rows: SourceEntry[]; catalog: EntryRecord[]; itemId: string; onChange: (rows: SourceEntry[]) => void }) {
  return <section className={panelClass}><PanelTitle title={definition.label} count={values.length} onAdd={() => onChange([...values, { owner_id: text(catalog[0]?.id), entry: { item_id: itemId, quantity: 1, ...(definition.chance ? { drop_chance: 100 } : {}) } }])} />
    <div className="mt-3 space-y-3">{values.map((row, index) => <SourceCard key={`${row.owner_id}-${index}`} title={label(catalog.find((owner) => owner.id === row.owner_id), row.owner_id)} onRemove={() => onChange(values.filter((_, rowIndex) => rowIndex !== index))}>
      ...
      <div className="grid gap-2 sm:grid-cols-2"><NumberField label="Quantity" value={row.entry.quantity} onChange={(quantity) => onChange(patchSourceEntry(values, index, { quantity }))} />{definition.chance && <NumberField label="Drop Chance %" value={row.entry.drop_chance} onChange={(drop_chance) => onChange(patchSourceEntry(values, index, { drop_chance }))} />}</div>
    </SourceCard>)}{values.length === 0 && <Empty text={`No ${definition.label.toLowerCase()} placements.`} />}</div>
  </section>;
}
...
function EconomyPanel({ packet, updateItem, setSources, simulation }: { packet: ItemPacket; updateItem: (patch: EntryRecord) => void; setSources: (key: string, value: unknown) => void; simulation: SimulationResult | null }) {
  ...
  return <div className="grid gap-4 lg:grid-cols-2"><section className={panelClass}><h2 className="font-semibold">Canonical Economy</h2><div className="mt-3 grid gap-3 sm:grid-cols-2"><NumberField label="Base Price" value={packet.item.base_price} onChange={(base_price) => updateItem({ base_price })} /><Select label="Base Currency" value={text(packet.item.base_currency_id)} options={packet.catalogs.currencies || []} reference="currencies" allowEmpty onChange={(base_currency_id) => updateItem({ base_currency_id })} /></div><div className="mt-3 grid gap-3 sm:grid-cols-2"><Fact label="Simulation Value" value={simulation?.metrics.value.toFixed(1) || "..."} /><Fact label="Economy Score" value={simulation?.metrics.economy.toFixed(1) || "..."} /></div></section>
    <section className={panelClass}><h2 className="font-semibold">Shop Price Overrides</h2><div className="mt-3 space-y-2">{shopRows.map((row, index) => <div key={text(row.id)} className="grid gap-2 rounded-md border p-3 sm:grid-cols-2"><div className="text-sm font-semibold">{label((packet.catalogs.shops || []).find((shop) => shop.id === row.shop_id), text(row.shop_id))}</div><NumberField label="Final Override" value={row.price_override} onChange={(price_override) => setSources("shop_inventory", patchRow(shopRows, index, { price_override }))} /></div>)}{shopRows.length === 0 && <Empty text="Add a shop source to tune final prices." />}</div></section></div>;
}
...
function ProgressionPanel({ packet, setSources }: { packet: ItemPacket; setSources: (key: string, value: unknown) => void }) {
  ...
  return <div className="grid gap-4 lg:grid-cols-2"><section className={panelClass}><h2 className="font-semibold">World Placements</h2><p className="mt-1 text-xs text-slate-500">Occupied POIs must be cleared from their current item before assignment.</p><div className="mt-3 space-y-2">{(packet.catalogs.pois || []).map((poi) => {
...
function Select({ label: fieldLabel, value, options, reference, onChange, allowEmpty = false }: { label: string; value: string; options: EntryRecord[]; reference?: string; onChange: (value: string) => void; allowEmpty?: boolean }) { const refreshed = useReferenceOptions(reference || ""); const resolved = refreshed.length ? refreshed : options; return <label className="block text-xs font-semibold uppercase text-slate-500">{fieldLabel}<select className={`${inputClass} mt-1 normal-case`} value={value} onChange={(event) => onChange(event.target.value)}>{allowEmpty && <option value="">Unassigned</option>}{resolved.map((option) => <option key={text(option.id)} value={text(option.id)}>{label(option)}</option>)}</select>{reference && <ReferenceManageLink reference={reference} onCreated={onChange} />}</label>; }
function NumberField({ label, value, onChange }: { label: string; value: unknown; onChange: (value: number | null) => void }) { return <label className="block text-xs font-semibold uppercase text-slate-500">{label}<input className={`${inputClass} mt-1`} type="number" value={value === null || value === undefined ? "" : text(value)} onChange={(event) => onChange(event.target.value === "" ? null : Number(event.target.value))} /></label>; }
function TextField({ label, value, onChange }: { label: string; value: unknown; onChange: (value: string) => void }) { return <label className="block text-xs font-semibold uppercase text-slate-500">{label}<input className={`${inputClass} mt-1 normal-case`} value={text(value)} onChange={(event) => onChange(event.target.value)} /></label>; }
...
function Empty({ text: value }: { text: string }) { return <div className="rounded-md border border-dashed border-slate-300 p-4 text-sm text-slate-500 dark:border-slate-700">{value}</div>; }
