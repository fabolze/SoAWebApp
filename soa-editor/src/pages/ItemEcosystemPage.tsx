...
function validatePacket(packet: ItemPacket): string[] {
  ...
  if (text(packet.item.equipment_set_id) && !["Weapon", "Armor", "Accessory"].includes(text(packet.item.type))) blockers.push("Only weapons, armor, and accessories can belong to an equipment set.");
  if (text(packet.item.equipment_set_id) && !(packet.catalogs.equipment_sets || []).some((row) => text(row.id) === text(packet.item.equipment_set_id))) blockers.push("Equipment set reference is missing.");
