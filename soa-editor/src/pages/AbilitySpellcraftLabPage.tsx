...
function IdentityPanel({ packet, updateAbility }: { packet: AbilityPacket; updateAbility: (patch: EntryRecord) => void }) {
  return <Panel id="ability-identity" title="Ability Identity" subtitle="Name the promise before assembling its mechanics."><div className="grid gap-3 md:grid-cols-2"><Field label="Name" value={packet.ability.name} onChange={(name) => updateAbility({ name, slug: displayText(packet.ability.slug) || generateSlug(name) })} /><Field label="Slug" value={packet.ability.slug} onChange={(slug) => updateAbility({ slug })} /><Field label="Icon Path" value={packet.ability.icon_path} onChange={(icon_path) => updateAbility({ icon_path })} /><TextArea className="sm:col-span-2" label="Description" value={packet.ability.description} onChange={(description) => updateAbility({ description })} /><TextArea className="sm:col-span-2" label="Design Intent" value={packet.ability.design_intent} onChange={(design_intent) => updateAbility({ design_intent })} /><TextArea className="sm:col-span-2" label="Counterplay" value={packet.ability.counterplay_notes} onChange={(counterplay_notes) => updateAbility({ counterplay_notes })} /><TextArea className="sm:col-span-2" label="Mastery" value={packet.ability.mastery_notes} onChange={(mastery_notes) => updateAbility({ mastery_notes })} /><TextArea className="sm:col-span-2" label="Presentation" value={packet.ability.presentation_notes} onChange={(presentation_notes) => updateAbility({ presentation_notes })} /><div className="md:col-span-2"><EditableTagList tags={packet.ability.tags} onChange={(tags) => updateAbility({ tags })} /></div></div></Panel>;
}
...
function LensPanel({ packet, lens, setLens, issues }: { packet: AbilityPacket; lens: Lens; setLens: (lens: Lens) => void; issues: { blockers: string[]; warnings: string[] } }) {
  ...
  const similar = localSimilarAbilities(packet);
  return <Panel id="ability-inspector" title="Spellcraft Lenses" subtitle="Read the same ability through different design questions."><div className="mb-3 flex flex-wrap gap-1">{(["sentence", "mix", "efficiency", "usage", "damage", "issues"] as Lens[]).map((value) => <button key={value} className={value === lens ? `${BUTTON_CLASSES.primary} ${BUTTON_SIZES.xs}` : `${BUTTON_CLASSES.outline} ${BUTTON_SIZES.xs}`} onClick={() => setLens(value)}>{value[0].toUpperCase() + value.slice(1)}</button>)}</div>
