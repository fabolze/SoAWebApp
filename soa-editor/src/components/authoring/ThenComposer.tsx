        ...
        <main className="space-y-4">
          ...
          <section className="space-y-2"><div className="flex items-center justify-between"><h3 className="font-semibold">{draft.shape === "constellation" ? "Captured ideas" : "What happens next"}</h3><span className="text-xs text-slate-500">{draft.steps.length} steps</span></div>
            {draft.steps.map((step, index) => <article id={`creation-flow-step-${step.id}`} key={step.id} className="scroll-mt-4 rounded-lg border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-900">
              ...
              {(["activate_location_variant", "activate_character_variant", "activate_item_variant"] as CreationFlowStepKind[]).includes(step.kind) && <label className="mt-2 block text-[10px] font-semibold uppercase text-slate-500">Variant identity<input aria-label={`Step ${index + 1} variant identity`} className={`${inputClass} mt-1 normal-case`} value={String(step.payload?.variantId ?? "")} onChange={(event) => setDraft(patchCreationFlowStep(draft, step.id, { payload: { ...(step.payload ?? {}), variantId: event.target.value } }))} placeholder="Stable variant id from the canonical record" /></label>}
