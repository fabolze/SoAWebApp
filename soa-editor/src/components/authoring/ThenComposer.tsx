      ...
      <div className="grid gap-4 p-4 xl:grid-cols-[240px_minmax(0,1fr)_300px]">
        ...
        <main className="space-y-4">
          <section className="rounded-lg border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
            <div className="grid gap-3 sm:grid-cols-[1fr_180px]"><label className="text-xs font-semibold uppercase text-slate-500">Flow title<input className={`${inputClass} mt-1 normal-case`} value={draft.title} onChange={(event) => update({ title: event.target.value })} /></label><label className="text-xs font-semibold uppercase text-slate-500">Shape<select aria-label="Composition shape" className={`${inputClass} mt-1 normal-case`} value={draft.shape} onChange={(event) => changeShape(event.target.value as CreationFlowDraft["shape"])}><option value="sequence">Sequence</option><option value="constellation">Constellation</option><option value="hybrid">Hybrid</option></select></label></div>
        ...
        <aside className="space-y-3">
          <section className="rounded-lg border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-900"><h3 className="text-sm font-semibold">Idea cards</h3><div className="mt-2 grid gap-2"><input aria-label="Idea label" className={inputClass} value={ideaLabel} onChange={(event) => setIdeaLabel(event.target.value)} placeholder="Name the idea" /><select aria-label="Idea kind" className={inputClass} value={ideaKind} onChange={(event) => setIdeaKind(event.target.value as CreationFlowRefKind)}>{IDEA_KINDS.map((kind) => <option key={kind} value={kind}>{kind.replace(/_/g, " ")}</option>)}</select><textarea aria-label="Idea direction" className={inputClass} rows={2} value={ideaDirection} onChange={(event) => setIdeaDirection(event.target.value)} placeholder="Creative direction (optional)" /><button type="button" className={`${BUTTON_CLASSES.violet} ${BUTTON_SIZES.sm}`} disabled={!ideaLabel.trim()} onClick={() => addIdea()}>Add idea card</button></div>
            <div className="mt-3 space-y-2">{draft.placeholders.map((placeholder) => {
              ...
              return <div key={placeholder.id} className="rounded border border-dashed border-violet-300 p-2 text-xs"><div className="flex items-start justify-between gap-2"><div><b>{placeholder.label}</b><span className="ml-1 text-violet-600">{placeholder.kind}</span></div><button type="button" className={`${BUTTON_CLASSES.danger} ${BUTTON_SIZES.xs}`} onClick={() => setDraft(removeCreationFlowPlaceholder(draft, placeholder.id))}>Remove idea</button></div>{placeholder.direction && <p className="mt-1 text-slate-600 dark:text-slate-300">{placeholder.direction}</p>}<label className="mt-2 block text-[10px] font-semibold uppercase text-slate-500">Canonical link<select aria-label={`Canonical link for ${placeholder.label}`} className={`${inputClass} mt-1 normal-case`} value={placeholder.promotedCanonicalId ?? ""} disabled={!catalog?.references[kind]} onChange={(event) => { const entry = entries.find((row) => row.id === event.target.value); setDraft(resolveCreationFlowPlaceholder(draft, placeholder.id, entry ? { kind, canonicalId: entry.id, label: entry.label } : undefined)); }}><option value="">Keep as local placeholder</option>{entries.map((entry) => <option key={entry.id} value={entry.id}>{entry.label}</option>)}</select></label><span className={`mt-1 block ${placeholder.promotedCanonicalId ? "text-emerald-700 dark:text-emerald-300" : "text-amber-700 dark:text-amber-300"}`}>{placeholder.promotedCanonicalId ? "Linked to an existing canonical record" : catalog?.references[kind] ? "Local placeholder" : "No canonical catalog exists for this idea type"}</span></div>;
            })}</div>
          </section>
