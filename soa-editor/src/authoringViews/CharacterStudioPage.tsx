    ...
    <div className="min-h-full bg-slate-100 p-4 dark:bg-slate-950">
      <div className="mx-auto max-w-[1800px] space-y-4">
        ...
        <div className="grid gap-4 xl:grid-cols-[280px_minmax(680px,1fr)_390px]">
          ...
          <Panel title="Content Library"><select className={inputClass} value={libraryKind} onChange={(event) => setLibraryKind(event.target.value)}>{["characters","abilities","quests","dialogues","encounters","shops","factions","characterclasses","locations","events","story_arcs"].map((kind) => <option key={kind}>{kind}</option>)}</select><p className="mt-2 text-xs text-slate-500">Use Connect to stage a sensible default connection, Add Beat to place content in the story, or drag it onto the canvas.</p><div className="mt-2 max-h-80 space-y-1 overflow-auto">{library.map((entry) => <DraggableCard key={displayText(entry.id)} kind={libraryKind} entry={entry} onAdd={["characters","abilities","quests","dialogues","encounters","shops","factions","characterclasses","locations"].includes(libraryKind)?()=>applyDrop(libraryKind,entry,"character"):undefined} onBeat={()=>applyDrop(libraryKind,entry,"trace")} />)}{library.length===0&&<div className="rounded border border-dashed border-slate-300 p-3 text-xs text-slate-500">No {libraryKind} exist yet. Create one, then return to connect it.</div>}</div><button className={`${active} mt-2 w-full`} onClick={() => createFocusedDraft(libraryKind)}>Create New {libraryKind.replace(/_/g," ")}</button></Panel></aside>
...
function Field({label,value,onChange}:{label:string;value:unknown;onChange:(v:string)=>void}){return <label className="block"><Caption>{label}</Caption><input aria-label={label} className={inputClass} value={editableText(value)} onChange={(e)=>onChange(e.target.value)}/></label>}
function TextArea({label,value,onChange}:{label:string;value:unknown;onChange:(v:string)=>void}){return <label className="block"><Caption>{label}</Caption><textarea aria-label={label} className={`${inputClass} min-h-20`} value={editableText(value)} onChange={(e)=>onChange(e.target.value)}/></label>}
