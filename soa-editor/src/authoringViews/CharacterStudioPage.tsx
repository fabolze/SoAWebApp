...
type CreatorStage = "identity" | "story" | "role" | "beats" | "review";
...
const generatedName = (value: unknown, id: unknown) => {
  const name = displayText(value).trim();
  const entryId = displayText(id).trim();
  return !name || name === "New Character" || name === entryId || /^[0-9a-f]{32}$/i.test(name) || /^01[A-Z0-9]{24}$/i.test(name);
};
...
export default function CharacterStudioPage() {
  ...
  const [creatorStage, setCreatorStage] = useState<CreatorStage>("identity");
  const [studioExpanded, setStudioExpanded] = useState(!isNew);
  ...
  useEffect(() => {
    setCreatorStage("identity");
    setStudioExpanded(!isNew);
  }, [id, isNew]);
  ...
  const hasIdentity = !generatedName(packet.character.name, packet.character.id);
  const hasStoryCore = Boolean(packet.story_profile && ["want", "need", "fear", "contradiction", "secret", "voice_notes"].some((key) => displayText(packet.story_profile?.[key]).trim()));
  const hasRole = Boolean(packet.combat_profile || packet.interaction_profile);
  const canEnterStudio = hasIdentity && hasStoryCore;
  const canReview = dirty && hasIdentity && packet.health.blockers.length === 0 && !(packet.combat_profile && !packet.character.class_id);
  const characterLabel = hasIdentity ? displayText(packet.character.name) : "New Character";
  const localState = isNew
    ? dirty ? "Draft saved locally" : "Not started"
    : dirty ? "Draft saved locally" : "Saved to project";
    ...
    <AuthoringPageShell>
      <div className="space-y-4">
        <Panel
          ...
          subtitle={`${localState} · ${packet.story_beats.length} story ${packet.story_beats.length === 1 ? "beat" : "beats"}`}
          ...
          actions={<div className="flex flex-wrap gap-2">{(!isNew || studioExpanded) && <><button className={studioMode === "individual" ? active : inactive} onClick={() => { setStudioMode("individual"); setSelectedIds([displayText(packet.character.id)]); }}>Individual</button><button className={studioMode === "ensemble" ? active : inactive} onClick={() => setStudioMode("ensemble")}>Ensemble</button></>}<button className={inactive} disabled={!dirty || saving} onClick={reset}>Reset Draft</button><button className={active} disabled={!canReview || saving} onClick={() => void preview()}>Review Character</button></div>}
        >
          <div className="text-xs font-semibold uppercase text-violet-600">Character Studio</div>
          <AuthoringHealthSummary blockers={isNew && !studioExpanded ? 0 : packet.health.blockers.length} warnings={isNew && !studioExpanded ? 0 : packet.health.warnings.length} dirty={dirty} saving={saving} isNew={isNew} />
          {notice && <div className="mt-3"><StatusNotice>{notice}</StatusNotice></div>}
        </Panel>
        <Panel id="character-steps" title="Create This Character" help="Begin with the person. Connections, mechanics, and implementation appear after identity and story core have something meaningful to build on.">
          <div className="grid gap-2 md:grid-cols-5">
            <QuickStep number="1" label="Identity" detail="Name, role, first impression" done={hasIdentity} active={creatorStage === "identity" && isNew && !studioExpanded} onClick={() => { setCreatorStage("identity"); setStudioExpanded(false); }} />
            <QuickStep number="2" label="Story Core" detail="Want, need, fear, contradiction" done={hasStoryCore} active={creatorStage === "story" && isNew && !studioExpanded} disabled={!hasIdentity} onClick={() => { if (!packet.story_profile) update("story_profile", newProfile(displayText(packet.character.id))); setCreatorStage("story"); setStudioExpanded(false); }} />
            <QuickStep number="3" label="Role" detail="How the player meets them" done={hasRole} active={creatorStage === "role" && isNew && !studioExpanded} disabled={!canEnterStudio} onClick={() => { setCreatorStage("role"); setStudioExpanded(true); setTab(packet.combat_profile ? "combat" : "interaction"); }} />
            <QuickStep number="4" label="Story Beats" detail="Entrances, decisions, changes" done={packet.story_beats.length > 0} active={creatorStage === "beats" && isNew && !studioExpanded} disabled={!canEnterStudio} onClick={() => { setCreatorStage("beats"); setStudioExpanded(true); setTab("presence"); }} />
            <QuickStep number="5" label="Review" detail="Check and save to project" done={!isNew && !dirty && packet.health.blockers.length === 0} active={creatorStage === "review" && isNew && !studioExpanded} disabled={!canEnterStudio} onClick={() => { setCreatorStage("review"); setStudioExpanded(true); setTab("pending"); }} />
          </div>
          <p className="mt-3 text-xs text-slate-500">Your draft stays local while you create. Project checks wait until you choose Review.</p>
        </Panel>
        {isNew && !studioExpanded && <Panel id="character-creator" title={creatorStage === "identity" ? "Meet the character" : "Find their dramatic core"} subtitle={creatorStage === "identity" ? "Start with the person, not their record." : "A few tensions are enough to make later scenes and relationships useful."}>
          {creatorStage === "identity" && <CharacterIdentityDraft packet={packet} updateCharacter={updateCharacter} onContinue={() => { if (!packet.story_profile) update("story_profile", newProfile(displayText(packet.character.id))); setCreatorStage("story"); }} />}
          {creatorStage === "story" && <CharacterStoryCoreDraft profile={packet.story_profile || newProfile(displayText(packet.character.id))} onChange={(storyProfile) => update("story_profile", storyProfile)} onBack={() => setCreatorStage("identity")} onContinue={() => setStudioExpanded(true)} canContinue={hasStoryCore} />}
        </Panel>}
        {(!isNew || studioExpanded) && <div className="grid min-w-0 gap-4 xl:grid-cols-[260px_minmax(620px,1fr)_360px]">
...
function CharacterIdentityDraft({ packet, updateCharacter, onContinue }: { packet: StudioPacket; updateCharacter: (patch: EntryRecord) => void; onContinue: () => void }) {
  const ready = !generatedName(packet.character.name, packet.character.id);
  return <div className="mx-auto max-w-3xl space-y-4">
    <div className="grid gap-4 md:grid-cols-2">
      <Field label="What is their name?" value={generatedName(packet.character.name, packet.character.id) ? "" : packet.character.name} onChange={(name) => updateCharacter({ name, slug: generateSlug(name) })} />
      <Field label="What do people call them?" value={packet.character.title} onChange={(title) => updateCharacter({ title })} />
    </div>
    <TextArea label="Who are they when the player first meets them?" value={packet.character.description} onChange={(description) => updateCharacter({ description })} />
    <details className="rounded-lg border border-slate-200 p-3 dark:border-slate-800">
      <summary className="cursor-pointer text-sm font-semibold">World and gameplay context</summary>
      <div className="mt-3 grid gap-3 md:grid-cols-3">
        <ReferenceChipPicker label="Class" reference="characterclasses" value={packet.character.class_id} onChange={(class_id) => updateCharacter({ class_id })} />
        <ReferenceChipPicker label="Faction" reference="factions" value={packet.character.faction_id} onChange={(faction_id) => updateCharacter({ faction_id })} />
        <ReferenceChipPicker label="Home" reference="locations" value={packet.character.home_location_id} onChange={(home_location_id) => updateCharacter({ home_location_id })} />
      </div>
    </details>
    <div className="flex justify-end"><button type="button" className={`${BUTTON_CLASSES.violet} ${BUTTON_SIZES.md}`} disabled={!ready} onClick={onContinue}>Continue to story core</button></div>
  </div>;
}

function CharacterStoryCoreDraft({ profile, onChange, onBack, onContinue, canContinue }: { profile: EntryRecord; onChange: (profile: EntryRecord) => void; onBack: () => void; onContinue: () => void; canContinue: boolean }) {
  const patch = (key: string, value: string) => onChange({ ...profile, [key]: value });
  return <div className="mx-auto max-w-4xl">
    <div className="grid gap-4 md:grid-cols-2">
      <TextArea label="What do they want?" value={profile.want} onChange={(value) => patch("want", value)} />
      <TextArea label="What do they actually need?" value={profile.need} onChange={(value) => patch("need", value)} />
      <TextArea label="What are they afraid of?" value={profile.fear} onChange={(value) => patch("fear", value)} />
      <TextArea label="What contradiction makes them human?" value={profile.contradiction} onChange={(value) => patch("contradiction", value)} />
      <TextArea label="What are they hiding?" value={profile.secret} onChange={(value) => patch("secret", value)} />
      <TextArea label="How do they sound?" value={profile.voice_notes} onChange={(value) => patch("voice_notes", value)} />
    </div>
    <div className="mt-4 flex justify-between gap-3">
      <button type="button" className={`${BUTTON_CLASSES.outline} ${BUTTON_SIZES.md}`} onClick={onBack}>Back to identity</button>
      <button type="button" className={`${BUTTON_CLASSES.violet} ${BUTTON_SIZES.md}`} disabled={!canContinue} onClick={onContinue}>Open character studio</button>
    </div>
  </div>;
}

function QuickStep({number,label,detail,done,active:activeStep=false,disabled=false,onClick}:{number:string;label:string;detail:string;done:boolean;active?:boolean;disabled?:boolean;onClick:()=>void}){return <button disabled={disabled} className={`rounded border p-3 text-left transition ${activeStep?"border-violet-500 bg-violet-50 ring-2 ring-violet-200 dark:bg-violet-950":done?"border-emerald-300 bg-emerald-50 dark:border-emerald-900 dark:bg-emerald-950":"border-slate-300 bg-white hover:border-violet-400 disabled:cursor-not-allowed disabled:opacity-45 dark:border-slate-700 dark:bg-slate-900"}`} onClick={onClick}><span className="text-[10px] font-bold uppercase text-slate-500">Step {number} {done?"Complete":""}</span><strong className="mt-1 block text-sm">{label}</strong><span className="mt-1 block text-xs text-slate-500">{detail}</span></button>}
