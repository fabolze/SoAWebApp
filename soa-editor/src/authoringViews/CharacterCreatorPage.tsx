...
function MultiPicker({ label, values, options, onChange }: { label: string; values: string[]; options: EntryRecord[]; onChange: (values: string[]) => void }) {
  return <div><Caption>{label}</Caption><div className="flex flex-wrap gap-1">{options.map((option) => { const id = displayText(option.id); const active = values.includes(id); return <button key={id} className={`rounded-full border px-2 py-1 text-xs ${active ? "border-blue-600 bg-blue-600 text-white" : "border-slate-300 dark:border-slate-700"}`} onClick={() => onChange(active ? values.filter((value) => value !== id) : [...values, id])}>{rowLabel(option, id)}</button>; })}</div></div>;
}
